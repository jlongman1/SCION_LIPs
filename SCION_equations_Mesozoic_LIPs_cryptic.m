function dy = SCION_equations_Mesozoic_LIPs_cryptic(t,y)
...
%%LOW Degassing Fluxes
%% Call for 'low' degassing scenario
flux_NW_Australia_Margin = 2.6e9 ;
flux_Karoo = 4.7e9 ;
flux_Ferrar = 2.5e10 ;
flux_CAMP = 1.1e11 ;
flux_Siberia = 2.9e11 ;
...
%%HIGH Degassing Fluxes
%% Call for 'high' degassing scenario

flux_NW_Australia_Margin = 3.2e9 ;
flux_Karoo = 3.6e12 ;
flux_Ferrar = 1.3e11 ;
flux_CAMP = 8.3e12 ;
flux_Siberia = 1.7e13 ;
flux_Emeishan = 1.7e12 ;
flux_Panjal = 1.3e10 ;
flux_Tarim = 5e9;

%%Running with fixed Gaussians half duration width for degassing

%%Call for 'low' scenario

LIP_CO2 = (flux_NW_Australia_Margin)*normpdf(t_geol,time_NW_Australia_Margin,5)...
    +(flux_Karoo)*normpdf(t_geol,time_Karoo,3.25)...
    +(flux_Ferrar)*normpdf(t_geol,time_Ferrar,0.25)...
    +(flux_CAMP)*normpdf(t_geol,time_CAMP,0.75)...
    +(flux_Siberia)*normpdf(t_geol,time_Siberia,1)...
    +(flux_Emeishan)*normpdf(t_geol,time_Emeishan,3.5)...
    +(flux_Panjal)*normpdf(t_geol,time_Panjal,15)...
    +(flux_Tarim)*normpdf(t_geol,time_Tarim,20);

%%Call for 'high' scenario

LIP_CO2 = (flux_NW_Australia_Margin)*normpdf(t_geol,time_NW_Australia_Margin,5)...
    +(flux_Karoo)*normpdf(t_geol,time_Karoo,0.235)...
    +(flux_Ferrar)*normpdf(t_geol,time_Ferrar,0.175)...
    +(flux_CAMP)*normpdf(t_geol,time_CAMP,0.3)...
    +(flux_Siberia)*normpdf(t_geol,time_Siberia,0.4)...
    +(flux_Emeishan)*normpdf(t_geol,time_Emeishan,0.5)...
    +(flux_Panjal)*normpdf(t_geol,time_Panjal,2.5)...
    +(flux_Tarim)*normpdf(t_geol,time_Tarim,2.5);

%Call for '50kyr scenario'
LIP_CO2 = (flux_NW_Australia_Margin)*normpdf(t_geol,time_NW_Australia_Margin,5)...
    +(flux_Karoo)*normpdf(t_geol,time_Karoo,0.05)...
    +(flux_Ferrar)*normpdf(t_geol,time_Ferrar,0.05)...
    +(flux_CAMP)*normpdf(t_geol,time_CAMP,0.05)...
    +(flux_Siberia)*normpdf(t_geol,time_Siberia,0.05)...
    +(flux_Emeishan)*normpdf(t_geol,time_Emeishan,0.05)...
    +(flux_Panjal)*normpdf(t_geol,time_Panjal,0.05)...
    +(flux_Tarim)*normpdf(t_geol,time_Tarim,0.05);
  
   
%To run with no degassing call this function
%LIP_CO2 = 0; 
 
LIP_CO2_d13c = -5 ;
%For LIP weathering call this 
LIPfactor = 7 ;

%To run without weathering call this
%LIPfactor = 1; 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Cryptic Degassing options in addition - from Black et al., 2024 Nat Geos
% Call for 'Crytpic' degassing scenario

%%%% choose for run
% TOTAL_CO2_INPUT_cryptic = interp1qr(forcings.LIP_CO2_t',forcings.LIP_CO2_input_low',t_geol) ; %%%% GROUP LOW
% TOTAL_CO2_INPUT_cryptic = interp1qr(forcings.LIP_CO2_t',forcings.LIP_CO2_input_mid',t_geol) ; %%%% MID POINT
TOTAL_CO2_INPUT_cryptic = interp1qr(forcings.LIP_CO2_t',forcings.LIP_CO2_input_high',t_geol) ; %%%% GROUP HIGH
% TOTAL_CO2_INPUT = 0 ; %%%% NULL

% %%%% with cryptic degassing LOW
% TOTAL_CO2_INPUT = TOTAL_CO2_INPUT + interp1qr(forcings.melt_envelopes(:,1),forcings.melt_envelopes(:,2),t_geol+16.65) ...
% + interp1qr(forcings.melt_envelopes(:,1),forcings.melt_envelopes(:,2),t_geol+66.42) ...
% + interp1qr(forcings.melt_envelopes(:,1),forcings.melt_envelopes(:,2),t_geol+56) ...
% + interp1qr(forcings.melt_envelopes(:,1),forcings.melt_envelopes(:,2),t_geol+252.24) ...
% + interp1qr(forcings.melt_envelopes(:,1),forcings.melt_envelopes(:,2),t_geol+30.51) ;

%%%% with cryptic degassing HIGH
TOTAL_CO2_INPUT_cryptic = TOTAL_CO2_INPUT_cryptic + interp1qr(forcings.melt_envelopes(:,1),forcings.melt_envelopes(:,3),t_geol+16.65) ...
+ interp1qr(forcings.melt_envelopes(:,1),forcings.melt_envelopes(:,3),t_geol+66.42) ...
+ interp1qr(forcings.melt_envelopes(:,1),forcings.melt_envelopes(:,3),t_geol+56) ...
+ interp1qr(forcings.melt_envelopes(:,1),forcings.melt_envelopes(:,3),t_geol+252.24) ...
+ interp1qr(forcings.melt_envelopes(:,1),forcings.melt_envelopes(:,3),t_geol+30.51) ;

%To run without Cryptic CO2 call this
TOTAL_CO2_INPUT_cryptic = 0 ;
%% 
...
%%%Introduce LIP maps as weatherable terranes%%%

if t_geol >= time_NW_Australia_Margin
    LIP_past_NW_Australia_Margin = LIPS.NW_Australia_Margin(:,:,key_past_index) ; 
    LIP_future_NW_Australia_Margin = LIPS.NW_Australia_Margin(:,:,key_future_index) ; 
else
    LIP_past_NW_Australia_Margin = land_past .* 0 ; 
    LIP_future_NW_Australia_Margin = land_future .* 0; 
end

if t_geol >= time_Karoo
    LIP_past_Karoo = LIPS.Karoo(:,:,key_past_index) ; 
    LIP_future_Karoo = LIPS.Karoo(:,:,key_future_index) ; 
else
    LIP_past_Karoo = land_past .* 0 ; 
    LIP_future_Karoo = land_future .* 0; 
end

if t_geol >= time_Ferrar
    LIP_past_Ferrar = LIPS.Ferrar(:,:,key_past_index) ; 
    LIP_future_Ferrar = LIPS.Ferrar(:,:,key_future_index) ; 
else
    LIP_past_Ferrar = land_past .* 0 ; 
    LIP_future_Ferrar = land_future .* 0; 
end

if t_geol >= time_CAMP
    LIP_past_CAMP = LIPS.CAMP(:,:,key_past_index) ; 
    LIP_future_CAMP = LIPS.CAMP(:,:,key_future_index) ; 
else
    LIP_past_CAMP = land_past .* 0 ; 
    LIP_future_CAMP = land_future .* 0; 
end

%%%% Siberian traps
if t_geol >= time_Siberia
    LIP_past_Siberia = LIPS.Siberia(:,:,key_past_index) ; 
    LIP_future_Siberia = LIPS.Siberia(:,:,key_future_index) ; 
else
    LIP_past_Siberia = land_past .* 0 ; 
    LIP_future_Siberia = land_future .* 0; 
end

if t_geol >= time_Emeishan
    LIP_past_Emeishan = LIPS.Emeishan(:,:,key_past_index) ; 
    LIP_future_Emeishan = LIPS.Emeishan(:,:,key_future_index) ; 
else
    LIP_past_Emeishan = land_past .* 0 ; 
    LIP_future_Emeishan = land_future .* 0; 
end


if t_geol >= time_Tarim
    LIP_past_Tarim = LIPS.Tarim(:,:,key_past_index) ; 
    LIP_future_Tarim = LIPS.Tarim(:,:,key_future_index) ; 
else
    LIP_past_Tarim = land_past .* 0 ; 
    LIP_future_Tarim = land_future .* 0; 
end


%%combine all LIP data
LIP_past = (LIP_past_NW_Australia_Margin + LIP_past_Karoo ...
            + LIP_past_Ferrar  + LIP_past_CAMP + LIP_past_Siberia  + LIP_past_Emeishan + LIP_past_Tarim);
LIP_future = (LIP_future_NW_Australia_Margin + LIP_future_Karoo ...
            + LIP_future_Ferrar  + LIP_future_CAMP + LIP_future_Siberia + LIP_future_Emeishan + LIP_future_Tarim );
...
%%%% LIP multiplier for weathering in the LIP boxes
CW_per_km2_past_noLIP = CW_per_km2_past.*(1 - LIP_past) ;
CW_per_km2_future_noLIP = CW_per_km2_future.*(1 - LIP_future) ;
CW_per_km2_past_LIP = CW_per_km2_past.*(LIPfactor-1).*LIP_past ;
CW_per_km2_future_LIP = CW_per_km2_future.*(LIPfactor-1).*LIP_future ;
CW_per_km2_tot = CW_per_km2_past_LIP + CW_per_km2_past_noLIP ;


%%%% CW total
CW_gran_past =  CW_per_km2_past_noLIP .* GRID_AREA_km2 ;
CW_gran_future =  CW_per_km2_future_noLIP .* GRID_AREA_km2 ;
CW_bas_past =  CW_per_km2_past_LIP .* GRID_AREA_km2 ;
CW_bas_future =  CW_per_km2_future_LIP .* GRID_AREA_km2 ;

%%%% world CW
CW_bas_past(isnan(CW_bas_past)==1) = 0 ;
CW_bas_future(isnan(CW_bas_future)==1) = 0 ;

CW_bas_sum_past = sum(sum(CW_bas_past)) ;
CW_bas_sum_future = sum(sum(CW_bas_future)) ;
CW_bas_tot = CW_bas_sum_past*contribution_past + CW_bas_sum_future*contribution_future ;

CW_gran_past(isnan(CW_gran_past)==1) = 0 ;
CW_gran_future(isnan(CW_gran_future)==1) = 0 ;
CW_gran_sum_past = sum(sum(CW_gran_past)) ;
CW_gran_sum_future = sum(sum(CW_gran_future)) ;
CW_gran_tot = CW_gran_sum_past*contribution_past + CW_gran_sum_future*contribution_future ;

CW_tot = CW_bas_tot + CW_gran_tot ;
...
basw_spatial = CW_bas_tot * ( pars.k_basw / basw_scale ) + 1e12*DEGASS ;
granw_spatial = CW_gran_tot * ( pars.k_granw / granw_scale ) ;
...
%%%% add fbiota
basw = basw_spatial * f_biota ;
granw = granw_spatial * f_biota ;
...
%%% Carbon dioxide
dy(3) = -locb - mocb + oxidw + ocdeg + ccdeg + carbw - mccb - sfw  + reductant_input + LIP_CO2 + TOTAL_CO2_INPUT_cryptic;
%
...
%%% delta_A * A
dy(16) = -locb*(  delta_locb ) -mocb*( delta_mocb ) + oxidw*delta_G + ocdeg*delta_G + ccdeg*delta_C + carbw*delta_C - mccb*delta_mccb - sfw*delta_mccb + reductant_input*-5 + LIP_CO2*LIP_CO2_d13c + TOTAL_CO2_INPUT_cryptic*-5;
%
...
if sensanal == 0
    ...
    workingstate.LIP_CO2_degass(stepnumber,1) = LIP_CO2 ;
    workingstate.LIP_area(stepnumber,1) = sum(sum(LIP_past)) ;
    workingstate.Cryptic(stepnumber,1) = TOTAL_CO2_INPUT_cryptic ;
    %%%%%%%% print time
    if pars.runcontrol == -2
...
if sensanal == 0
    ...
    if pars.finishgrid == 0
        if t_geol > next_stamp || t_geol == 0
            ...
            gridstate.CW_gran(:,:,pars.gridstamp_number) = CW_per_km2_past_noLIP ; %%% t/km2/yr
            gridstate.CW_bas(:,:,pars.gridstamp_number) = CW_per_km2_past_LIP ; %%% t/km2/yr
            gridstate.CWcarb(:,:,pars.gridstamp_number) = CWcarb_past ;
            gridstate.CW(:,:,pars.gridstamp_number) = CW_per_km2_past_LIP + CW_per_km2_past_noLIP ; %%% t/km2/yr
            gridstate.EPSILON(:,:,pars.gridstamp_number) = EPSILON_past * 1e6 ; %%% t/km2/yr
            gridstate.LIP(:,:,pars.gridstamp_number) = LIP_past;
